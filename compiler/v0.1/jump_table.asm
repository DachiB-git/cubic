...
StrLit -> "CC"
------------------------------------------------
prog -> TyDS VaDS FuDS                          // first(prog) = {typedef, char, short, int, uchar, ushort, uint, bool, TyNa, func}
                                                // follow(prog) = {$}
TyDS -> TyD TyDS | eps                          // first(TyDS) = {typedef, eps}
                                                // follow(TyDS) = {char, short, int, uchar, ushort, uint, bool, TyNa, func}
...
St -> id = GenE ;                              
St -> FuncNa (ArgS) ;  
MatchedElse -> else ChainIf | eps               // first(MatchedElse) = {else, eps}
                                                // follow(MatchedElse) = {Na, if, while, do, FuncNa}
ChainIf -> { StS }                              // first(ChainIf) = {{, if}
| if (GenE) { StS } MatchedElse                 // follow(ChainIf) = {Na, if, while, do, FuncNa}
                                                
GenE -> FuncNa (ArgS) | JointE                  // first(GenE) = {StrLit, FuncNa, Na, *, &, -, !, (, Num, true, false}
                                                // follow(GenE) = {, , ;, ), ]}
...
RT -> * F RT | / F RT | eps                     // first(RT) = {*, /, eps}
                                                // follow(RT) = {+, -, ), >, >=, <, <=, ==, !=, &&, ||, ], , , ;, )}
F -> id | -F | !F | *F | &F | Ty PDeco F |      // first(F) = {id, -, !, *, &, (, Num, true, false}
( RelE ) | Num | true | false
                                                // follow(F) = {*, /, +, -, ), >, >=, <, <=, ==, !=, &&, ||, ], , , ;, )}
